...
| Migration workflow | Diff/generate/apply/rollback tracking exists for SQL files; dry-run previews and generated preflight checks are available for risky column changes, required additive columns, field-level unique constraints, new unique columns, and compound unique constraints; generated rollback covers additive operations and column type/nullability reversal | Broaden preflight coverage for index/data-shape edge cases |
